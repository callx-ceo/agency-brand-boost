...
export const EmailTemplateSelector = ({ onCustomizationChange }: { onCustomizationChange?: (customization: any) => void }) => {
  const [activeTab, setActiveTab] = React.useState<"call_score" | "recommended_actions">("call_score");
  const [customization, setCustomization] = React.useState({
    primaryColor: activeTab === "call_score" ? "#667eea" : "#10b981",
    headerText: activeTab === "call_score" ? "Call Performance Score" : "Recommended Actions",
    footerText: activeTab === "call_score" 
      ? "This is an automated performance report. Keep improving! 🚀"
      : "These recommendations are generated based on your performance data. Keep growing! 🌱"
  });
  ...
  return (
    <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="w-full">
      <TabsList className="grid w-full grid-cols-2">
        <TabsTrigger value="call_score">Call Score Email</TabsTrigger>
        <TabsTrigger value="recommended_actions">Recommended Actions</TabsTrigger>
      </TabsList>
